namespace KinectV2MouseControl
{
    ...
    /// supplied explicitly by the host: one hand points, clicks, drags and right clicks; the
    /// opposite hand scrolls and swipes while its fist clutch is engaged. Priority, highest first:
    ///
    ...
    ///  2. Grip on the pointer hand (drag). Owned by KinectCursor because it is welded to the
    ///     click anchoring, and surfaced here as GestureContext.IsDragActive. While a drag is
    ///     live the lasso stands down.
    ///  3. Lasso on the pointer hand. Right click.
    ///  4. Secondary-hand fist clutch. Decides SecondaryGestureArmed. Releasing it resets the swipe
    ///     history, the scroll engagement and neutral, and any part-accumulated scroll.
    ///  5. Swipe with the clutched secondary hand. Evaluated before scroll, because a swipe is the
    ///     more specific claim on the same hand. Suppressed while a drag is live.
    ///  6. Scroll with the clutched secondary hand. Suppressed for the duration of the swipe
    ...
    public class GestureEngine
    {
        ...
        /// <summary>
        /// What the clutched secondary hand is doing, for the diagnostics readout.
        /// </summary>
        ...
        public void Update(GestureContext context)
        {
            ...
            // 3. Pointer hand: right click. Stands down on its own while dragging.
            lasso.Update(context, router);

            // 4. Secondary-hand clutch. Only that hand inside the zone may engage it.
            HandSnapshot secondary = context.Hands[context.SecondaryHandIndex];
            ...
            if (wasArmed != clutch.IsArmed)
            {
                ...
                RuntimeLog.Write(GestureContext.DescribeHand(context.SecondaryHandIndex)
                    + (clutch.IsArmed ? " clutch armed" : " clutch released"));
            }
